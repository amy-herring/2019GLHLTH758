...
#SEX=1 male, 2 female
o_data <- o_data[o_data$SEX!=9,]
# remove rows with Unspecified gender (9)
o_data$SEX=factor(o_data$SEX, levels=c(1,2),labels=c("Male","Female")) 
# define factor

o_data$GEST_1=o_data$GEST; o_data$BWTG_1=o_data$BWTG
o_data$GEST_1[o_data$GEST_C>50]=NA
o_data$GEST_1[o_data$GEST_C<20]=NA
o_data$BWTG_1[o_data$BWTG_C<500]=NA
o_data$GEST_1[o_data$GEST_C<25 & o_data$BWTG_1>2000]=NA 
# now center them at typical values of 39 weeks and 3300g
o_data$BWTG_C=o_data$BWTG_1-3300
o_data$GEST_C=o_data$GEST_1-39 
...
```{r intmodel}
model2 = lm(BWTG_1~GEST_1+SEX+GEST_1*SEX, data=birth_data)
summary(model2)
```
...
female_slope <- model2$coefficients["GEST_1"]
male_slope <- model2$coefficients["GEST_1"] +  model2$coefficients["GEST_1:SEXMale"]
...
ggplot(birth_data_red, aes(x=GEST,y=BWTG,color=SEX)) + geom_point(aes(alpha=0.1)) + 
...
```{r centeredmodel}
model3 = lm(BWTG_1~GEST_C+SEX+GEST_C*SEX, data=birth_data)
summary(model3)
...
female_slope3 <- model3$coefficients["GEST_C"]
male_slope3 <- model3$coefficients["GEST_C"] +  model3$coefficients["GEST_C:SEXMale"]
...
```{r makeplot3, out.width="50%"}
# Plot some of the observed values and the predictions, sampling from original data 
# (so ggplot doesn't run super slowly)

ggplot(birth_data_red, aes(x=GEST_C,y=BWTG,color=SEX)) + geom_point(aes(alpha=0.1)) + 
