...
o_data <- read.csv("~/Documents/TEACHING/vitalstats/Yr1117Birth.csv", na.strings=c("99", "9999"))
...
# very basic data cleaning (you should do better!)
...
#make a plot
library(ggplot2)
ggplot(data = durhamb17, mapping = aes(x = GEST_C, y = BWTG_C)) +
  geom_point(alpha=1/10,color="blue") + xlab("Gestational age (weeks)") + ylab("Birth weight (g)") + 
  ggtitle("Durham Co, NC Births, 2011-2017")
...
library(haven)
lbw=read_dta("http://www.stata-press.com/data/r13/gsem_lbw.dta")

glimpse(lbw)

ggplot(data = lbw, mapping = aes(x = bwt)) +
  geom_histogram(binwidth = 200) + xlab("Birth weight (g)") 
